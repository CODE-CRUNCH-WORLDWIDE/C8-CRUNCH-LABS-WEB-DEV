...
```mermaid
flowchart TD
  A["User clicks plus one button"] --> B["setCount is called"]
  B --> C["React schedules a re-render"]
  C --> D["Counter function runs again"]
  D --> E["New JSX tree is created"]
  E --> F["React reconciles with previous tree"]
  F --> G["Real DOM updated to match"]
```
*One click walks the full render cycle from state update to DOM update.*
...
```mermaid
sequenceDiagram
  participant Build as Astro Build Server
  participant HTML as Static HTML
  participant Browser as Browser
  participant React as React Hydration
  Build->>HTML: Server render Foo to HTML
  HTML->>Browser: Ship static HTML snapshot
  Browser->>React: Load Foo JavaScript when visible
  React->>Browser: Hydrate onto existing HTML
  React->>React: Compare server output to client output
  Note over React: Mismatch throws error and re-renders
```
*Server-rendered HTML and client hydration must agree, or React discards and re-renders.*
