...
```mermaid
flowchart LR
  A["Browser requests App jsx"] --> B["Vite dev server"]
  B --> C["esbuild transforms on demand"]
  C --> D["Serve as native ES module"]
  E["Vite build command"] --> F["esbuild transform"]
  F --> G["Rollup bundles and tree shakes"]
  G --> H["Optimized production chunks"]
```
*Dev serves transformed modules directly; prod hands the same transform to Rollup for bundling.*
...
sequenceDiagram
  ...
  participant Browser as Browser HMR Runtime
  FS->>Server: File saved App jsx
  Server->>Server: Identify affected modules
  Server->>Browser: WebSocket message with new source
  Browser->>Browser: Replace old module exports
  Browser->>Browser: Run accept callback or propagate up
```
*How a saved file reaches the browser without a full page reload.*
