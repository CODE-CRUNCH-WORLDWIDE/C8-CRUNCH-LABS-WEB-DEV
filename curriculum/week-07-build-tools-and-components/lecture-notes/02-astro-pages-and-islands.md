...
```mermaid
flowchart TD
  A["src pages folder"] --> B["index astro maps to slash"]
  A --> C["about astro maps to slash about"]
  A --> D["projects bracket slug astro"]
  D --> E["getStaticPaths returns list of slugs"]
  E --> F["one static HTML file per slug"]
```
*The filesystem is the router: every file under src pages becomes a URL.*
...
sequenceDiagram
  ...
  Build->>HTML: Render ProjectSearch with empty query
  HTML->>Browser: Ship static HTML plus JS bundle reference
  Browser->>Browser: Render list with no JavaScript running
  Browser->>IO: User scrolls near search box
  IO->>Browser: Fetch ProjectSearch bundle
  Browser->>Browser: Hydrate and attach onChange handler
```
*The search box stays inert HTML until it scrolls into view, then hydrates.*
