...
**C.** HMR replaces individual modules in the running page without a full reload. State that lives in JavaScript variables (form input, scroll position, selected tab, React component state in many cases) is preserved across the update. The Vite HMR API is at <https://vitejs.dev/guide/api-hmr.html>.
...
**C.** The frontmatter runs at build time on the Astro server. Variables it declares are interpolated into the template via `{}` expressions. **None of the frontmatter ships to the browser** unless you explicitly add a `<script>` block in the template body.
...
**B.** `<slot />` is the canonical "child content goes here" element, mirroring the HTML Living Standard's `<slot>` semantics for Web Components. Astro borrowed the name and the meaning. Reference: <https://docs.astro.build/en/basics/astro-components/#slots>.
...
**C.** `client:visible` uses `IntersectionObserver` and hydrates when the component scrolls into view. `client:load` hydrates immediately on page load; `client:idle` hydrates on the next `requestIdleCallback`; `client:lazy` is not a real directive.
...
**B.** The functional updater form `setCount(prev => prev + 1)` receives the most recent state, even if multiple updates are queued. `setCount(count + 1)` uses whatever value `count` had at the time the handler was created, which may be stale if multiple updates run in a row. Reference: <https://react.dev/learn/queueing-a-series-of-state-updates>.
