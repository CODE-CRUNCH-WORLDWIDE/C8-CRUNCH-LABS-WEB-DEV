...
- **Not a media-query festival.** Most of what learners reach for `@media` to fix is fixable with `clamp()`, `auto-fit`, or container queries. We do use `@media` — but only where it earns its keep.
