...
```mermaid
flowchart TD
  subgraph Container["Flex container sets"]
    A1["flex-direction"]
    A2["flex-wrap"]
    A3["justify-content"]
    A4["align-items"]
  ...
  subgraph Child["Flex child sets"]
    B1["order"]
    B2["flex-grow flex-shrink flex-basis"]
    B3["align-self"]
  end
```
*Container properties decide the shared rules; child properties override them one item at a time.*
...
```mermaid
flowchart TD
  A["Layout to build"] --> B{"Just a row or a column?"}
  B -->|"Yes"| C["Flexbox"]
  B -->|"No - rows and columns relate"| D["Grid"]
  C --> E["Card row that wraps on a phone"]
  D --> F["Header sidebar main footer holy grail"]
```
*The one-sentence decision rule, drawn as a tree.*
