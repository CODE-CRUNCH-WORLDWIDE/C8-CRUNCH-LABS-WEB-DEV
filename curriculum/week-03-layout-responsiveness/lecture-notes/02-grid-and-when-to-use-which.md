...
```mermaid
flowchart TD
  A["Responsive decision"] --> B{"Keyed to viewport or component?"}
  B -->|"Viewport"| C["media query"]
  B -->|"Component size"| D["container query"]
  C --> E["Page-level: does the page have a sidebar"]
  D --> F["Component-level: does this card have room"]
```
*Choosing between a media query and a container query.*
...
```mermaid
flowchart TD
  A["What shape is the layout?"] --> B{"Single row or column?"}
  B -->|"Yes"| C["Flexbox"]
  B -->|"No"| D{"Same-size items that wrap?"}
  D -->|"Yes"| E["Either - Grid auto-fit is fewer lines"]
  D -->|"No"| F{"Regions spanning rows and columns?"}
  F -->|"Yes"| G["Grid"]
  F -->|"No"| H["Container query on Flexbox or Grid"]
```
*Walking the decision rule from layout shape to the right tool.*
