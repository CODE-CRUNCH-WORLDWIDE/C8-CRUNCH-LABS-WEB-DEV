...
flowchart TD
  ...
  Html --> Body["body"]
  Head --> Title["title"]
  Title --> TitleText["Demo"]
  Body --> H1["h1"]
  H1 --> Text1["Hello"]
  H1 --> Em["em"]
  H1 --> Text2["exclamation mark"]
  Em --> World["world"]
```
*The parsed HTML page as a tree of typed nodes rooted at document.*
...
```mermaid
flowchart TD
  Start["Need to change an element"] --> Q1{"About how it looks"}
  Q1 -->|Yes| A["Use a class"]
  Q1 -->|No| Q2{"About platform state such as disabled or checked"}
  Q2 -->|Yes| B["Use the property"]
  Q2 -->|No| Q3{"About HTML metadata such as data or aria"}
  Q3 -->|Yes| C["Use the attribute"]
  Q3 -->|No| D["Use style.setProperty with a custom property"]
```
*Which of the four interfaces to reach for, in decision order.*
