...
```mermaid
flowchart TD
  W["window"] -->|capture| Doc["document"]
  Doc -->|capture| Html["html"]
  Html -->|capture| Body["body"]
  Body -->|capture| Form["form"]
  Form -->|capture| Btn["button target phase"]
  Btn -->|bubble| Form
  Form -->|bubble| Body
  Body -->|bubble| Html
  Html -->|bubble| Doc
  Doc -->|bubble| W
```
*The same click travels down the tree in capture, then back up in bubble.*
...
```mermaid
flowchart TD
  Click["User clicks inside the list"] --> Bubble["Event bubbles up to the list listener"]
  Bubble --> Closest["event.target.closest todo-delete"]
  Closest -->|No match| Ignore["Return early, ignore the click"]
  Closest -->|Match found| Row["Find the parent row and its id"]
  Row --> Delete["deleteTodo with that id"]
```
*One listener on the parent routes every click to the right handler via closest.*
