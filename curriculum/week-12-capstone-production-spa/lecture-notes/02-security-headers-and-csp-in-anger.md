...
```mermaid
flowchart LR
  A["Deploy report-only"] --> B["Audit console reports"]
  B --> C["Add missing origins"]
  C --> D["Promote to enforcing"]
  D --> E["Redeploy and verify"]
```

*The iterative-tighten loop every team uses to ship CSP without breaking the app.*
