...
```mermaid
flowchart TD
  A["Push or open PR"] --> B["vitest job"]
  A --> C["playwright job"]
  A --> D["lighthouse job"]
  B --> E{"All checks pass"}
  C --> E
...
*Branch protection turns three parallel CI jobs into a single merge gate.*
...
```mermaid
flowchart LR
  A["Minutes 0-2 Product demo"] --> B["Minutes 2-7 Architecture tour"]
  B --> C["Minutes 7-10 Reflection"]
```

*The 10-minute walkthrough moves from the live product to the code to the retro.*
