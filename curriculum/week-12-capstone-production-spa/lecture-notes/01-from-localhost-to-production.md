...
```mermaid
flowchart TD
  A["Request for /profile"] --> B{"Who serves it"}
  B -->|"Vite dev server"| C["Re-serve index.html"]
  C --> D["Client router handles /profile"]
  B -->|"Production host default"| E["Look for dist/profile.html"]
  E --> F["File not found"]
  F --> G["404 response"]
```

*Why the same route works on localhost but 404s in production without a rewrite.*
...
```mermaid
flowchart TD
  A["Decision 1: hosting platform"] --> B["Decision 2: IdP"]
  B --> C["Decision 3: project archetype"]
  C --> D["Decision 4: framework"]
  D --> E["Decision 5: state management"]
  E --> F["Decision 6: data persistence"]
```

*Each Monday-Tuesday decision constrains the ones after it.*
