...
```mermaid
flowchart TD
  A["New piece of state"] --> B{"Does it come from a server"}
  B -->|Yes| C["Server state - TanStack Query"]
  B -->|No| D{"Should the user bookmark or share it"}
  D -->|Yes| E["URL state - router params"]
  D -->|No| F["Local UI state - useState"]
```
*The three diagnostic questions that route a piece of state to its layer.*
...
```mermaid
flowchart TD
  S1["Step 1: useState in one component"] --> S2["Step 2: useState lifted to parent"]
  S2 --> S3["Step 3: useReducer for complex transitions"]
  S3 --> S4["Step 4: Context for ambient values"]
  S4 --> S5["Step 5: external store Zustand"]
  S5 -.demote when only one reader remains.-> S1
```
*The local state ladder — start at the bottom rung, step up only when a concrete pain point forces it.*
