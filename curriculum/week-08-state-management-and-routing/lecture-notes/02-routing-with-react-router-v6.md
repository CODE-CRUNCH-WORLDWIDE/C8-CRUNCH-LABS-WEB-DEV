...
```mermaid
flowchart TD
  Root["RootLayout - renders Outlet"] --> Home["Home - index route"]
  Root --> Books["BooksLayout - renders Outlet"]
  Root --> About["About"]
  Books --> Index["BooksIndex - books index"]
  Books --> Detail["BookDetail - books colon bookId"]
  Books --> New["NewBookForm - books new"]
```
*Nested route objects map to a nested layout tree; each layout keeps rendering while its Outlet swaps the matched child.*
...
```mermaid
flowchart TD
  A["User navigates to dashboard"] --> B["requireAuth loader runs"]
  B --> C{"Is the user authenticated"}
  C -->|No| D["throw redirect to login"]
  C -->|Yes| E["Loader returns the user"]
  D --> F["Login page renders"]
  E --> G["Dashboard component renders"]
```
*A loader-level auth check redirects before the protected route ever renders — no flash of protected UI.*
