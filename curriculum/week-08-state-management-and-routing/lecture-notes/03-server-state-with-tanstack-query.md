...
stateDiagram-v2
  ...
  Fresh --> Inactive: no components subscribed
  Stale --> Inactive: no components subscribed
  Inactive --> Stale: component resubscribes
  Inactive --> GarbageCollected: gcTime elapses
  GarbageCollected --> [*]
```
*A cached query's lifecycle: freshness governed by staleTime, cleanup governed by gcTime.*
...
```mermaid
flowchart LR
  A["User submits form"] --> B["mutation.mutate"]
  B --> C["mutationFn posts to server"]
  C --> D["onSuccess runs"]
  D --> E["invalidateQueries books"]
  E --> F["Subscribers refetch books list"]
  F --> G["New book appears in UI"]
```
*A successful mutation invalidates the related query, which triggers a refetch that syncs the UI with the server.*
