...
sequenceDiagram
  ...
  participant Fixture as loggedInPage fixture
  participant IdP as Sign-in form
  participant T1 as Test one
  participant T2 as Test two
  Worker->>Fixture: first test on this worker requests loggedInPage
  Fixture->>IdP: fill username and password, submit
  IdP-->>Fixture: redirect back with session
  Fixture-->>T1: inject already logged in page
  Fixture-->>T2: reuse same page, no new login
```
*Login runs once per worker; every test on that worker reuses the same authenticated page.*
...
```mermaid
flowchart TD
  A["Test fails in CI"] --> B["Open the Playwright report artifact"]
  B --> C["Click into the failed test"]
  C --> D["Open the trace with show-trace"]
  D --> E["Step through timeline to find the bad action"]
  E --> F["Reproduce locally with debug mode if possible"]
  F --> G["Push a fix and trust the CI re-run"]
```
*The trace-first workflow for diagnosing a flaky end-to-end test without re-running blind.*
