...
```mermaid
flowchart TD
  A["Choosing a query variant"] --> B{"Is the element already in the DOM"}
  B -->|"Yes"| C["Use getBy - throws if missing"]
  B -->|"No, checking it is absent"| D["Use queryBy - returns null if missing"]
  B -->|"No, waiting for it to appear"| E["Use findBy - retries until timeout"]
```
*Picking the right Testing Library query variant based on timing.*
...
sequenceDiagram
  ...
  Suite->>Server: beforeAll starts server
  loop Each test file
    Test->>Server: request handled by base or override handler
    Test->>Server: afterEach resets handlers
  end
  Suite->>Server: afterAll closes server
```
*How the MSW server starts once, resets between tests, and closes after the whole suite.*
