...
```mermaid
flowchart TD
  A["Arrange - set up the world"] --> B["Act - perform one action"]
  B --> C["Assert - check the result"]
```
*The three ordered sections every well-shaped test follows.*
...
```mermaid
flowchart TD
  A["New test candidate"] --> B{"Bug-detection benefit vs total cost"}
  B -->|"High benefit low cost"| C["Write it - e.g. billing calculation unit test"]
  B -->|"High benefit medium cost"| D["Write it - e.g. login form component test"]
  B -->|"Low benefit high cost"| E["Skip it - e.g. full tree snapshot test"]
```
*Whether a test earns its keep depends on benefit divided by total cost, not on habit.*
