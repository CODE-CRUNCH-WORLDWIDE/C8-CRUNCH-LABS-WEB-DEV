...
```mermaid
flowchart TD
  A["Resource need identified"] --> B{"This page or next page"}
  B -->|"This page, third-party origin"| C["preconnect"]
  B -->|"This page, critical resource"| D["preload"]
  B -->|"Next page navigation"| E["prefetch"]
  C --> F["Open handshake early"]
  D --> G["Start download now at high priority"]
  E --> H["Download later at low priority"]
```
*Choosing among preconnect, preload, and prefetch based on when the resource is needed.*
...
```mermaid
flowchart TD
  A["Web font requested"] --> B{"font-display value"}
  B -->|block| C["Blank text up to 3s, then swap"]
  B -->|swap| D["Fallback text immediately, then swap"]
  B -->|fallback| E["Blank 100ms, fallback, then swap if within 3s"]
  B -->|optional| F["Blank 100ms, fallback, no swap this load"]
```
*How the four font-display values change what the user sees before the web font arrives.*
