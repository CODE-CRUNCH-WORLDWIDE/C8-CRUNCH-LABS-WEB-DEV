...
```mermaid
flowchart TD
  A["Parse HTML - build DOM"] --> B["Parse CSS - build CSSOM"]
  B --> C["Compute layout"]
  C --> D["Paint each layer"]
  D --> E["Composite the layers"]
  C -.re-layout on shift.-> C
```
*The five-stage rendering pipeline; a layout shift loops back into layout itself.*
...
flowchart LR
  ...
  C --> D["Resource load time"]
  D --> E["Element render delay"]
  E --> F["LCP reported"]
```
*The four phases that sum to the reported LCP value.*
