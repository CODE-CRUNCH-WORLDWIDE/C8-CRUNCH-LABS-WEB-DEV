...
```mermaid
flowchart LR
  A["One user's browser"] -->|"onLCP onINP onCLS"| B["web-vitals library"]
  B -->|"sendBeacon"| C["Your analytics endpoint"]
  C --> D["Your dashboard, own domain only"]
  A -.opted into reporting.-> E["Chrome"]
  E --> F["CrUX, whole-web dataset"]
  F --> G["PageSpeed Insights"]
```
*Per-user metrics fork into your own analytics pipeline and, separately, into Chrome's public CrUX dataset.*
...
```mermaid
flowchart TD
  A["1. Baseline in PageSpeed Insights"] --> B["2. Identify worst vital"]
  B --> C["3. Diagnose in DevTools Performance Insights"]
  C --> D["4. Apply one named fix"]
  D --> E["5. Re-profile in DevTools"]
  E --> F["6. Confirm with Lighthouse"]
  F --> G["7. Ship and watch CrUX"]
  G -.still bad, repeat.-> B
```
*The seven-step measure-diagnose-fix-confirm workflow, looped until the vital is green.*
