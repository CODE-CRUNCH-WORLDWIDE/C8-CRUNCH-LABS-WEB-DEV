...
```mermaid
flowchart TD
  A["Choose auth token model"] --> B{"Single monolith one database"}
  B -->|Yes| C["Sessions"]
  B -->|No| D{"Multiple services or mobile client"}
  D -->|Yes| E["JWTs"]
  D -->|No| F{"Public third-party API"}
  F -->|Yes| G["OAuth with JWTs"]
  F -->|No| H["Either works - start with framework default"]
```

*None of these branches is universally right - the decision matrix, not fashion, should drive the choice.*
...
sequenceDiagram
  ...
  Client->>AS: Use refresh token A
  AS->>Client: Issue new access and refresh token B
  Note over AS: Token A now marked used
  Attacker->>AS: Replay stolen refresh token A
  AS->>Attacker: Reject - token already used
  AS->>AS: Revoke entire token family
```

*Whichever party presents the already-used token second is the breach signal that triggers a full family revocation.*
...
**Clerk.** <https://clerk.com/>. React-first developer experience. Drop-in `<SignIn />` widget. Free tier suitable for learner projects. The friendliest DX in the space.
