...
sequenceDiagram
  ...
  U->>C: Visits app
  C->>AS: Redirect to authorize endpoint
  AS->>U: Show login page
  U->>AS: Authenticate
  AS->>C: Redirect back with code and state
  C->>AS: POST code to token endpoint
  AS->>C: Return access token and id token
  C->>RS: Call API with bearer token
```

*The eight-step authorization code dance from redirect to first API call.*
