...
```mermaid
flowchart TD
  A["Every ECMAScript value"] --> B["Primitive seven types"]
  A --> C["Object one type"]
  B --> D["Undefined Null Boolean"]
  B --> E["String Number BigInt Symbol"]
  C --> F["Objects arrays functions dates"]
```
*The spec's entire type universe: seven primitives plus one object type, nothing else.*
...
sequenceDiagram
  ...
  O-->>G: return 42
```
*Each call pushes a frame; each return pops one, in strict last-in-first-out order.*
