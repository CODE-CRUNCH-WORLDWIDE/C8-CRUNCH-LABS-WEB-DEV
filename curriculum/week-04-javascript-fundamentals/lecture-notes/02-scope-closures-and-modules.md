...
```mermaid
flowchart TD
  A["inner scope d equals 4"] --> B["middle scope c equals 3"]
  B --> C["outer scope b equals 2"]
  C --> D["global scope a equals 1"]
```
*Name resolution walks the scope chain innermost first, stopping at the first match.*
...
sequenceDiagram
  participant Caller
  participant makeCounter
  participant increment
  Caller->>makeCounter: call makeCounter
  makeCounter-->>Caller: return increment with captured count
  Caller->>increment: call counter
  increment->>increment: count equals count plus 1
  increment-->>Caller: return count
  Note over makeCounter: frame already popped, count still alive
```
*The captured count outlives makeCounter's own call, kept alive by the returned closure.*
