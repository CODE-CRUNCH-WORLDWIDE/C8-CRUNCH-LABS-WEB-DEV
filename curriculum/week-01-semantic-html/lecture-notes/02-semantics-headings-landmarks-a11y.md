...
*A typical nesting of the seven landmark elements inside a page.*
...
```mermaid
flowchart TD
  A["Need to convey meaning to assistive tech"] --> B{"Does a native HTML element already do this"}
  B -->|Yes| C["Use the native element"]
  B -->|No| D["Add the right ARIA attribute"]
```

*The first rule of ARIA, as a decision path.*
