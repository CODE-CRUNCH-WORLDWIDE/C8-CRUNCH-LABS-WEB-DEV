...
```mermaid
flowchart TD
  A["Two rules target the same element"] --> B{"Same origin and importance?"}
  B -- No --> C["Higher origin and importance wins"]
  B -- Yes --> D{"Same specificity?"}
  D -- No --> E["Higher specificity wins"]
  D -- Yes --> F["Rule written later in source wins"]
```
*The cascade's three rounds — the first round with a clear winner ends the comparison.*
...
```mermaid
flowchart TD
  A["Body sets color ink - inherited by paragraphs"] --> B["p rule sets color navy - specificity 0-0-0-1"]
  B --> C["dot lede rule sets color crimson - specificity 0-0-1-0"]
  C --> D["ID main dot lede rule sets color forestgreen - specificity 0-1-1-0"]
  D --> E["Winner: forestgreen"]
```
*Each rule that matches the first paragraph beats the last on rising specificity, not on being written first.*
