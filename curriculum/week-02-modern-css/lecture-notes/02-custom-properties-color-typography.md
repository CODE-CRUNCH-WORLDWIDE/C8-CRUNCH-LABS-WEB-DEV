...
```mermaid
flowchart TD
  A["Need to pick or mix a color"] --> B{"Exact brand hex given?"}
  B -- Yes --> C["Use the hex value directly"]
  B -- No --> D{"Need predictable lightness across a palette?"}
  D -- Yes --> E["Use oklch for the base color and its shades"]
  D -- No --> F["Use hsl for a one-off intuitive pick"]
  E --> G{"Deriving hover, active, or a tint from an existing token?"}
  F --> G
  G -- Yes --> H["Use color-mix in oklch"]
```
*Which color tool to reach for, depending on what you already know and what you are deriving.*
...
```mermaid
flowchart TD
  A["OS reports prefers-color-scheme"] --> B{"Dark mode requested?"}
  B -- No --> C["Root tokens stay at light values"]
  B -- Yes --> D["Media query reassigns the root tokens to dark values"]
  C --> E["Components read the same shared tokens"]
  D --> E
  E --> F["Body, links, and rules update with zero duplicated CSS"]
```
*Four root tokens, one media query, and every component that reads them updates together.*
