...
stateDiagram-v2
  [*] --> Untouched
  Untouched --> Invalid: Blur while invalid
  Untouched --> Valid: Blur while valid
  Invalid --> Invalid: Input still invalid
  Invalid --> Valid: Input becomes valid
  Valid --> Invalid: Blur while invalid again
```

*The show-on-blur, clear-on-input state machine for a single field's error message.*
...
```mermaid
flowchart TD
  A["User clicks Next"] --> B{"Every field in current step valid"}
  B -->|"No"| C["Render inline errors and error summary"]
  C --> A
  B -->|"Yes"| D["Copy step FormData into state values"]
  D --> E["Hide current step"]
  E --> F["Show next step"]
  F --> G["Focus next step heading"]
```

*How goToStep validates, advances, and re-anchors focus for a multi-step form.*
