...
```mermaid
flowchart TD
  A["Writing a form control"] --> B{"Can I use label-for or wrap the input"}
  B -->|"Yes"| C["Use label-for or wrapping"]
  B -->|"No"| D{"Does the visible label live elsewhere on the page"}
  D -->|"Yes"| E["Use aria-labelledby"]
  D -->|"No"| F{"Is there genuinely no visible label"}
  F -->|"Yes"| G["Use aria-label"]
  F -->|"No"| H["Stop and add a real visible label"]
```

*The four-question decision tree for choosing how to label any form control.*
...
sequenceDiagram
  ...
  participant J as Submit Handler
  U->>F: Click submit button
  F->>B: Run the submission algorithm
  B->>B: Check constraint validity of every field
  alt Any field invalid
    B->>U: Show native validation bubble
  else All fields valid
    B->>F: Fire submit event
    F->>J: Submit event handler runs
    J->>J: event.preventDefault stops navigation
    J->>J: Read entries with new FormData
  end
```

*What happens between a click on Submit and your handler seeing the data.*
